...
2. **Serial number extraction**
   ...
   Implementations supporting only v3 issuer TBSes (the typical MOICA case) SHOULD pin the version block bytes (`[0xa0, 0x03, 0x02, 0x01, 0x02]` at TBS offsets `[4..8]`) as constants. Implementations supporting other X.509 versions MUST document the supported version set and the corresponding canonical serial-number offsets.
