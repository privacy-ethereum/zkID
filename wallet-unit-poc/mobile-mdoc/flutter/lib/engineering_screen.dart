...
import 'package:mopro_flutter_bindings/src/rust/third_party/openac_mdoc_mobile_app.dart'
    show
        BenchmarkResults,
        PresentationResult,
        ProofResult,
...
class TaskResult {
  ...
  final PresentationResult? presentation;
...
class _E2EProofWorkflowScreenState extends State<E2EProofWorkflowScreen> {
  ...
  Future<TaskResult> _executeStep(
    ...
    switch (taskType) {
      case ProofTaskType.setupMdoc:
        final t = DateTime.now();
        final msg = await setupMdocKeys(documentsPath: documentsPath);
        return TaskResult(
          taskType: taskType,
          success: true,
          message: msg,
      ...
      case ProofTaskType.proveMdoc:
        final pr = await proveMdoc(documentsPath: documentsPath);
        return TaskResult(
          taskType: taskType,
          success: true,
          proofResult: pr,
      ...
      case ProofTaskType.reblindMdoc:
        final pr = await reblindMdoc(documentsPath: documentsPath);
        return TaskResult(
          taskType: taskType,
          success: true,
          proofResult: pr,
      ...
      case ProofTaskType.verifyMdoc:
        final t = DateTime.now();
        final ok = await verifyMdoc(documentsPath: documentsPath);
        return TaskResult(
          taskType: taskType,
          success: ok,
          verifyResult: ok,
      ...
      case ProofTaskType.verifyPresentation:
        final t = DateTime.now();
        final p = await verifyPresentation(documentsPath: documentsPath);
        return TaskResult(
          taskType: taskType,
          success: p.linked && p.agePredicateSatisfied,
          verifyResult: p.linked && p.agePredicateSatisfied,
          presentation: p,
          clientTimingMs: DateTime.now().difference(t).inMilliseconds,
        );
    }
  ...
  Future<void> _runE2EWorkflow() async {
    ...
    const steps = [
      ProofTaskType.setupMdoc,
      ProofTaskType.setupShow,
      ProofTaskType.generateBlinds,
      ProofTaskType.proveMdoc,
      ProofTaskType.reblindMdoc,
      ProofTaskType.proveShow,
      ProofTaskType.reblindShow,
      ProofTaskType.verifyMdoc,
      ProofTaskType.verifyShow,
      ProofTaskType.verifyPresentation,
    ];
  ...
  String _taskTypeToDisplayName(ProofTaskType type) {
    return switch (type) {
      ProofTaskType.setupMdoc => 'Setup MDOC Keys',
      ProofTaskType.setupShow => 'Setup Show Keys',
      ProofTaskType.generateBlinds => 'Generate Shared Blinds',
      ProofTaskType.proveMdoc => 'Prove MDOC',
      ProofTaskType.proveShow => 'Prove Show',
      ProofTaskType.reblindMdoc => 'Reblind MDOC',
      ProofTaskType.reblindShow => 'Reblind Show',
      ProofTaskType.verifyMdoc => 'Verify MDOC',
      ProofTaskType.verifyShow => 'Verify Show',
      ProofTaskType.verifyPresentation => 'Verify Presentation',
    };
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('OpenAC mDL Age Verification'),
        actions: [
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: _buildOperationButton(
                    taskType: ProofTaskType.setupMdoc,
                    label: 'Setup MDOC',
                    icon: Icons.key,
                    color: Colors.blue,
                  ),
            ...
            // ── Step 3: mDL Credential (MDOC) ────────────────────────────────────────────────
            _buildSectionHeader('Step 3: mDL Credential (MDOC)', Icons.assignment),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
                  child: _buildOperationButton(
                    taskType: ProofTaskType.proveMdoc,
                    label: 'Prove MDOC',
                ...
                Expanded(
                  child: _buildOperationButton(
                    taskType: ProofTaskType.reblindMdoc,
                    label: 'Reblind MDOC',
                    icon: Icons.sync,
                    color: Colors.green,
                  ),
            ...
            // ── Step 4: Show ───────────────────────────────────────────────
            _buildSectionHeader('Step 4: Show (Age Predicate)', Icons.visibility),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
            ...
            Row(
              children: [
                Expanded(
                  child: _buildOperationButton(
                    taskType: ProofTaskType.verifyMdoc,
                    label: 'Verify MDOC',
            ...
            const SizedBox(height: 12),
            _buildOperationButton(
              taskType: ProofTaskType.verifyPresentation,
              label: 'Verify Presentation',
              icon: Icons.verified_user,
              color: Colors.indigo,
            ),

            // ── Step 6: Age Verification Result ────────────────────────────
            if (_results['verifyPresentation']?.presentation != null) ...[
              const SizedBox(height: 24),
              _buildSectionHeader('Step 6: Age Verification Result', Icons.badge),
              const SizedBox(height: 12),
              _buildAgeResultCard(_results['verifyPresentation']!.presentation!),
            ],
  ...
  Widget _buildAgeResultCard(PresentationResult p) {
    final ok = p.agePredicateSatisfied;
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(ok ? Icons.verified : Icons.gpp_bad,
                    color: ok ? Colors.green.shade800 : Colors.red.shade800,
                    size: 32),
                const SizedBox(width: 10),
                Text(
                  ok ? 'AGE VERIFIED' : 'AGE CHECK FAILED',
                  style: TextStyle(
                    fontSize: 22,
                    fontWeight: FontWeight.bold,
                    color: ok ? Colors.green.shade800 : Colors.red.shade800,
            ...
            // On its own line deliberately. agePredicateSatisfied true with
            // linked false is an honest mdoc proof paired with an independently
            // forged Show over self-chosen claim values — the attack the
            // split-proof design exists to stop. One combined tick would hide it.
            Row(
              children: [
                Icon(p.linked ? Icons.link : Icons.link_off,
                    size: 18,
                    color:
                        p.linked ? Colors.green.shade700 : Colors.red.shade700),
          ...
          children: [
            ...
            Table(
              ...
              children: [
                ...
                _sizeRow('MDOC Proving Key', r.mdocProvingKeyBytes),
                _sizeRow('MDOC Verifying Key', r.mdocVerifyingKeyBytes),
                _sizeRow('Show Proving Key', r.showProvingKeyBytes),
                _sizeRow('Show Verifying Key', r.showVerifyingKeyBytes),
                _sizeRow('MDOC Proof', r.mdocProofBytes),
                _sizeRow('Show Proof', r.showProofBytes),
                _sizeRow('MDOC Witness', r.mdocWitnessBytes),
                _sizeRow('Show Witness', r.showWitnessBytes),
              ],
            ),
            const SizedBox(height: 12),
            // Rendered outside the table: it is an outcome, not a timing, and a
            // "1 ms" row would read as a duration.
            Row(
              children: [
                Icon(
                  r.agePredicateSatisfied ? Icons.verified : Icons.gpp_bad,
                  size: 18,
                  color: r.agePredicateSatisfied
                      ? Colors.green.shade700
                      : Colors.red.shade700,
                ),
                const SizedBox(width: 6),
                Text(
                  r.agePredicateSatisfied
                      ? 'Age predicate satisfied'
                      : 'Age predicate NOT satisfied',
                  style: TextStyle(
